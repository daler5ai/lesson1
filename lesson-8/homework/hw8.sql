...
10)---Using Orders and Invoices tables, list customer IDs 
--who placed orders in 2023 January to find those who did not have invoices.
...
FROM Orders 
LEFT JOIN Invoices  ON customerid = customerid
WHERE orderdate >= '2023-01-01' 
  ...
  Combine all product names from Products and 
  ...
  --Combine all product names from Products
  --and Products_Discounted without duplicates.
  ...
  14)--Using Products table,
  group products based on price: 'Low' (<100), 'Mid' (100-500), 'High' (>500). 
  ...
  15)--Using City_Population table,
  use Pivot to show values of Year column in seperate columns ([2012], [2013])
...
SELECT productname, COUNT(productid) AS times_sold
FROM Products 
JOIN Sales  ON productid = productid
