...
select count (*)
as number_of_raws from Customers
...
11)--Write a query to calculate the total sales and average sales for each product category from the Sales table.
	...
	13)--Write a query that finds the highest and lowest 
	--Salary by department in the Employees table.(DeptID is enough, if you don't have DeptName).
	...
	15)--Write a query to show the AVG salary and
	--COUNT(*) of employees working in each department.
	--(DeptID is enough, if you don't have DeptName).

	select departmentName, avg(salary) as avg_salary,count(*) as number_emp
	from Employees
	group by DepartmentName
	...
	select saledate, sum(saleamount)
	from sales
	group by SaleDate
...
select * from Employees
	...
	--and then filter categories
	--with an average price greater than 150.
	...
	--then filter the results to include
	--only Customers with total sales over 1500.
	...
	22)--Write a query to find the total and 
	--average salary of employees in each department,
	--and filter the output to include only departments
	--with an average salary greater than 65000.
	...
	23)--Write a query to find total amount 
	--for the orders which weights
	--more than $50 for each customer along with their least purchases.
...
select custid, case when freight > 50 then freight else 0 end as amount, freight  from tsql2012.sales.orders
order by custid
...
24)--Write a query that calculates the total sales
    ...
    SUM(Quantity * totalamount) AS TotalSales, 
